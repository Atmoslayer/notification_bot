...
```
Далее бот запускается командой:
```commandline
python3 main.py           
```
...
```
Для создания контейнера используйте `docker run` с указанием имени контейнера через `--name` и указанием пути к .env файлу чере- `--env-file`:
```commandline
docker run --name notification_bot --env-file=./.env -it notification_bot
```
После создания контейнер с ботом будет запущен и готов к работе.
## Цель проекта
